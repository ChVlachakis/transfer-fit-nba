...
st.set_page_config(page_title="Transfer Fit NBA — Player Cards v2", layout="wide")
...
# MAIN APP
# -------------------------
st.title("Transfer Fit NBA — Player Cards v2")
...
with col_card:
    with st.container(border=True, height=PANEL_HEIGHT):
        ...
        sel_map = pool[["player_id", "player_name", "team_abbr", "primary_pos", "fit_score_pctile"]].copy()
        sel_map["label"] = (
            sel_map["player_name"].astype(str)
            + " | " + sel_map["team_abbr"].astype(str)
            + " | " + sel_map["primary_pos"].astype(str)
            + " | Fit " + sel_map["fit_score_pctile"].round(1).astype(str)
        )
        sel_map = sel_map.sort_values("fit_score_pctile", ascending=False)
        ...
        if pid is None:
            ...
            if prow.empty:
                ...
                k1.metric("FitScore (MVP)", f"{fit_pct:.1f}" if fit_pct is not None else "—")
